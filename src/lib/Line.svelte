<script lang="ts">
    ...
    function handleBrush(event: any) {
      ...
      if (selection) {
          ...
          const startDate = xScale.invert(startPixel);
          const endDate = xScale.invert(endPixel);
          yearRange = [startDate, endDate];
      } else {
          yearRange = xScale.domain();
      }
    }
  ...
  <svg {width} {height} class="line">
    ...
    <g class="points">
          {#each data as point (point.x)}
              <circle
                  cx={xScale(point.x)} cy={yScale(point.y)} r="2" fill="steelblue"
              />
          {/each}
      </g>
